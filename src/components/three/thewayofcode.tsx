...
interface ComponentWithMetadata extends React.FC<HankiesInTheWindProps> {
    metadata: typeof metadata;
}

const HankiesInTheWind: ComponentWithMetadata = ({
    initialZoom = 6,
}) => {
    ...
    useEffect(() => {
        ...
        const createInterferenceField = (
            sources: WaveSourceProps[],
            size: number,
            resolution: number,
            time: number
        ) => {
            ...
            for (let i = 0; i <= resolution; i++) {
                ...
                for (let j = 0; j <= resolution; j++) {
                    ...
                    sources.forEach(
                        ({ position: [sx, sy, sz], frequency, amplitude, phase }) => {
                            const dx = x - sx;
                            const dz = z - sz;
                            const distance = Math.sqrt(dx * dx + dz * dz);
                            height +=
                                Math.sin(distance * frequency - time * 5 + phase) *
                                amplitude *
                ...
                for (let j = 1; j < resolution; j++) {
                    ...
                    if (heightDiff > 0.2) {
                        ...
                        geometry2.setAttribute(
                            "position",
                            new THREE.Float32BufferAttribute(points2, 3)
                        );
        ...
        const animate = () => {
            ...
            const field2 = createInterferenceField(
                sources2,
                0.8 * 4,
                32,
                time + 0.33
            ...
            const field3 = createInterferenceField(
                sources3,
                0.8 * 4,
                32,
                time + 0.66
